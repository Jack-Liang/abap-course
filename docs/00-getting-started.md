...
常用端口：
- `3200` SAP GUI：SAP 客户端与服务器通信
- `3300` RFC：ABAP 系统间通信协议
- `30213` HANA：HANA 数据库端口，可通过数据库软件如 HANA Studio 、DBeaver 等连接
- `8443` Cloud Connector：与 BTP 平台通信的接口，默认不开启
- `50000/50001` HTTP/HTTPS：ABAP 系统对外提供 http/https 服务的端口

注意**不要用大写 `-P`**（随机分配端口会导致 SAP GUI 等客户端连不上）。

4. 首次启动：该镜像**未预加载数据（initial load）**，启动后第一次打开事务码和程序会明显偏慢，这是由于程序尚未编译导致的，运行一次后会正常。
...
>
> **许可证：** 仅限学习/演示用途；ABAP 许可证有效期约 3 个月，到期后用 SAP*（客户端 000）进 SLICENSE 取硬件 key，到 minisap 页面选择 A4H 系统下载新许可，通过事务码 SLICENSE 导入，或挂载至 `/opt/sap/ASABAP_license` 更新。**未到期却报许可无效**，多半是容器 MAC 变了、硬件 key 跟着变——启动命令加固定 `--mac-address` 可防（见上方参数表）。
>
> **提示：** `DDIC` / `SAP*` 仅用于系统管理（比如 DEVELOPER 密码过期时用 SU01 重置），日常开发请始终用 DEVELOPER 登录。
...
> **个人自学者注意：** 两款桌面版 GUI 都挂在 SAP Support Portal 的下载区，需要 SAP 客户/合作伙伴的 S-user 授权才能下载——没有公司渠道的自学者常卡在这一步。你也可以尝试通过 [SAP 试用版下载页](https://www.sap.com/products/try-sap/trials-downloads.html?search=SAP+GUI) 用普通 SAP 账号注册下载桌面版 GUI；若仍拿不到，用浏览器版即可完成全部课程（做法见本节末尾）。版本与系统要求以 [SAP GUI 官方页面](https://pages.community.sap.com/topics/gui/family)为准。
...
```
- 如果出现 **403 Forbidden**，说明该服务未启用——仍需想办法装桌面版，然后在 SICF 中启用该服务。
- 如果不修改 hosts 文件，需要把浏览器地址栏中的 `vhcala4hci` 修改为 `localhost`。

> 课程演示与截图均以桌面版 SAP GUI（英文界面）为准；浏览器版事务码与功能一致，个别布局、快捷键有差异。
...
![SE38 SAPBC_DATA_GENERATOR](https://cdn.jsdelivr.net/gh/jack-liang/abap-course-assets@main/00-getting-started/SE38_SAPBC_DATA_GENERATOR.png)
...
![SE38 ZABAPGIT_STANDALONE](https://cdn.jsdelivr.net/gh/jack-liang/abap-course-assets@main/00-getting-started/SE38_ZABAPGIT_STANDALONE.png)
...
| 包 | 名字 | 类型 | 放什么 |
|----|------|------|--------|
| 课程仓库包 | **`ZABAP_COURSE`**（描述随意，如 "ABAP Course"） | 普通开发包`ZABAP_COURSE`或本地包`$ABAP_COURSE`， | abapGit 仓库——随本仓库分发的课程对象 |
| 个人练习包 | 自定，如 `ZMY_PRACTICE` | 普通包或本地包 `$TMP` 均可 | 你跟课敲的练习代码、课后作业 |

- 选**本地包**：不能放在`$TMP`下，需新建`$`开头本地包，**无需传输请求**。
- 普通开发包：系统会弹出Workbench创建提示，确认即可，单机练习无需真实传输。
- 练习场景优先用`$TMP`：对象不进入传输请求，操作简单。
- 若练习对象需要走传输（第17课内容），创建普通包。
- 两类包分开，避免练习代码混入abapGit仓库，防止Pull受干扰（参考第4节约定）。

### 3. 在线克隆本仓库

1. 运行 `ZABAPGIT_STANDALONE`，点击 **New Online**；
2. 填写：
   - Git repository URL：`https://github.com/Jack-Liang/abap-course.git.
   - Package：`ZABAP_COURSE` 或 `$ABAP_COURSE`
   - Branch：`master`
3. 点击 **Clone**，之后进入仓库页面点击 **Pull** 拉取全部对象；
4. Pull 完成后对象列表应为绿色（全部激活）。

![abapGit 新建在线仓库](https://cdn.jsdelivr.net/gh/jack-liang/abap-course-assets@main/00-getting-started/ABAPGIT_ONLINE_PULL.png)
...
![STRUST 导入证书](https://cdn.jsdelivr.net/gh/jack-liang/abap-course-assets@main/00-getting-started/STRUST.png)

- 方法二（绕开）：GitHub 上 Code → Download ZIP 下载本仓库，abapGit 菜单选择 **Import from ZIP** 离线导入，效果相同（只是之后无法直接 Pull 更新）。

![GitHub 下载本仓库 ZIP](https://cdn.jsdelivr.net/gh/jack-liang/abap-course-assets@main/00-getting-started/GITHUB_DOWNLOAD_ZIP.png)

![abapGit 导入 ZIP](https://cdn.jsdelivr.net/gh/jack-liang/abap-course-assets@main/00-getting-started/abapGit_IMPORT_ZIP.png)
