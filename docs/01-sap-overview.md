...
- **最大命中数（Maximum no. of hits）**：查未知规模的表先限 100 条以内；
- **排序**：结果列表双击列标题快速排序；
...
### 6. SAP Easy Access 与收藏夹

- 命令栏输入 `/n` + 事务码：无论当前在哪个界面，强制回到该事务码初始屏幕；
- `/o` + 事务码：**新开会话窗口**打开（最多 6 个），写代码时一边 SE38 一边 SE16 查数就靠它；
...
!!! tip "查大表先限量"
    ...
    生产系统一张表动辄千万行，SE16 不设最大命中数直接 F8，轻则等半天，重则拖垮系统。把"先限 100"变成条件反射。
