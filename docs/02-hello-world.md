...
仓库版 `zac_hello_world` 在结尾还多一段**课程环境自检**：逐张检查 SFLIGHT 模型的 SCARR / SPFLI / SFLIGHT / SBOOK / SCUSTOM / STRAVELAG 是否存在、有无数据、各多少条（表存在性查数据字典 DD02L 的激活版本，条数用动态表名 `COUNT(*)`）：
...
SCARR     : 存在，18 条数据
SPFLI     : 存在，3640 条数据
SFLIGHT   : 存在，14640 条数据
...
```

若某行显示"不存在"或"暂无数据"，按提示回第0课生成/检查演示数据——后续每一课都依赖这些表（条数以你系统实际生成的数据为准）。
