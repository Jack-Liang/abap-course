...
[第9课](../../docs/09-function-module.md)的 Function Group 与 Function Module **参考源码**。两个对象已随仓库正式入库（`src/zac_flight_utils.fugr.*`，abapGit Pull 即得），本目录保留作阅读与自建练习的对照——自建时按课文步骤用自己的名字，对照本目录：
...
4. Export：`EV_FOUND TYPE ABAP_BOOL`、`EV_DURATION_MIN TYPE I`、`EV_DISTANCE TYPE S_DISTANCE`、`EV_CITYFROM TYPE S_FROM_CIT`、`EV_CITYTO TYPE S_TO_CITY`；
