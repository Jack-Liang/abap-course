...
**本目录是过渡产物**：请作者把下列源码逐个粘贴进 SAP 系统、画好 Screen 100、全部激活并自测通过后，用 **abapGit pull** 让系统生成正式的 `src/zac_flight_manager.prog.abap` / `.prog.xml` / dynpro XML 等入库文件——**届时本目录整目录删除**。所以这里只放纯 `.abap` 源码，不手写任何 XML。
